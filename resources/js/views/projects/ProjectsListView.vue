<template>
  <v-container fluid class="py-12">
    ...
    <v-row dense v-for="project in projectList" :key="project.id" class="mb-6">
      <v-col cols="12">
        <v-card class="elevation-2">
          ...
          >
          <v-card-text>
            <p class="text-subtitle-2">Required Skills: {{ project.skills }}</p>
          </v-card-text>

          <v-card-actions>
            <v-btn @click="apply(project.id)" class="text-secondary" outlined>
              <v-icon class="mr-2">mdi-application</v-icon> Apply to this
              project
            </v-btn>
            <v-spacer />
            <v-btn
